...
knitr::include_graphics("blogposts/11_3_20_OER/img/OER.png")
```
</div>
<div class="col-sm-8">

# [Looking back on a few years of OER development](https://crumplab.github.io/blogposts/11_3_20_OER/11_3_20_OER.html)

November 3, 2020

Some new and old thoughts on developing open educational resources while at Brooklyn College of CUNY. 
...
<div class="col-sm-4">
```{r, echo=F, out.width="50%"}
knitr::include_graphics("blogposts/7_20_20_HistPsychonomics/img/psychonomics.png")
```
</div>
<div class="col-sm-8">

# [Revisiting the history of Psychonomics from a eugenics lens](https://crumplab.github.io/blogposts/7_20_20_HistPsychonomics/7_20_20_HistPsychonomics.html)
...
2018-07-26, last compiled: `r Sys.Date()`

update: here is this example in a github repo [https://github.com/CrumpLab/rmslickquiz](https://github.com/CrumpLab/rmslickquiz)
...
1. downloaded the slickquiz jquery library [https://github.com/jewlofthelotus/SlickQuiz](https://github.com/jewlofthelotus/SlickQuiz)
2. Looked at it for a while. `index.html` loads right up and gives you a quiz.
3. Edit the quiz content in the `slickQuiz-config.js` file. That's in a javascript JSON variable, ugh. 
4. The index.html file has everything that you need set-up, and pipes the quiz in through the .js file. Edit that file, change the quiz.
5. To use in R,  I converted the json variable to YAML, then saved that file. YAML is much easier to write in.
6. In R, read in YAML, then write it back as a .js file (with JSON) so the jquery library can be friends with it.
7. Include important parts of the head from index.html as an include to the yaml of this page, it loads css files
8. Use htmltools to insert another .html file that contains the important parts from the body in the index. knit.
9. So, now I can insert a quiz. Next step improve the workflow, and make it work with more than one quize block per page.
...
```{r testquiz, echo=TRUE, eval=TRUE, cache=FALSE}
library(yaml)
library(rjson)
...
htmltools::includeHTML("quizzes/html/body.html")
```
...
# Adding meta tags to R Markdown websites for social media

2018-07-26, last compiled: `r Sys.Date()`

R Markdown seems to be a little bit inconsistent when attempting to add meta tags into html documents. Here's what I did to get them to work for twitter on this website. You can look at the [.rmd file for this post here](https://github.com/CrumpLab/CrumpLab.github.io/blob/master/Blog.Rmd). Or read this.

1. You can include .html into the head in the yaml, the yaml for this blog looks like this, and the special sauce is in the `header_blog.html`
...
output:
  html_document:
    ...
    code_folding: hide
    includes:
       in_header: [g_analytics.html,header_blog.html]
    css: blog.css
bibliography: [Crump.bib, typing.bib]
```

2. I found some example code for writing the .html to include, so I put this in the r setup, just underneath the yaml in this document. It writes the .html to your website folder. Also, code shows some use of meta tags for twitter.
...
1. How do I change the width of the main text? 

    a. Put a style block just after the yaml, % works best for compatibility between browser and mobile
  
    b. would be better to make a .css file and add it in.
  
    c. need to work out different widths for browsers and phones
...
2. Changing font-size
    ...
    a. add blog.css to yaml css
  
    b. make h1, h2, h3, h4 classes
  
    c. adjust font sizes, e.g.: `font-size: 15pt;`
...
1.  A lecture, often full of worked out examples of how to do arithmetic, putting numbers into letters and solving for X.
2. Some homeworks, where students are given partial information, and have to solve for X
3. Midterms and Final exams, written in multiple choice format, where there are numerically correct answers for solving the arithmetic properly.
4. A bit of some other stuff, hopefully.
...
1. Midterm 10-15%, or maybe up to 25% of final grade
2. Final 25% of final grade (lowest possible at BC)
3. Weekly online quizzes 15%?
4. Weekly Lab activities 30%
5. Some extra credit 2-5%
...
**Lab assignments**. I'm building an entire lab manual right now. It's massive hand holding, asking students to copy/paste R code to solve problems. They need this. I'm not teaching computer programming, because that is next level scary. They will get points for showing up, as measured by handing in a pre-made R-Markdown document that contains three sections. 
  ...
  a. Section 1 is the copy/paste part where they just copy the lab manual code in and compile. 1 point for doing that. NO FEAR.
  b. Section 2 is a generalization assignment. They slightly modify the R code to do something different, but very similar to the example. There will be probably be lots of failed compilation. That's ok, I show them how to set `eval=FALSE` when it doesn't work. They hand in their succesful or unsuccesful attempt. 1 point for trying, no matter what. NO FEAR.
  c. Section 3 is a written answer(s) to  one or more structured questions. This comes with a clear grading rubric. The TAs grade and give feedback on this most important part of the lab. Considering making this a group assignment, where if the students want, they can form a group, and hand in the exact same written answer. Everyone in the group gets the same grade. Reduces load on TA. Option for group work might increase motivation for some. This part is graded with fear, in the sense of no-do-overs in the lab. BUT...it feeds into my plans for the midterm and final...
...
Why ask me? One of my science hobbies is to use the task of typing in the laboratory (or online using Amazon's Mechanical Turk) to study how basic cognitive processes shape skilled performance. Over the years I've published a few papers on typing [@BehmerCrunchingbigdata2017; @BehmerSpatialknowledgeskilled2017; @CrumpEpisodiccontributionssequential2010; @CrumpHierarchicalcontrolskilled2010; @CrumpPreventioncorrectionposterror2013; @crump_warning:_2010; @liu_you_2010; @logan_cognitive_2010; @LoganHierarchicalcontrolcognitive2011; @logan_left_2009; @YamaguchiSpeedaccuracytradeoff2013]. And, I've read way too many papers on typing.
...
5. Typing strings is cue driven. The strings act as the cue for how you typed them last time. Recency matters even for experts, and the recent strings that you typed can modify how you type them and similar strings in the present [@CrumpEpisodiccontributionssequential2010]
...
```{r, results='asis', warning=FALSE,message=FALSE, cache=TRUE, eval=FALSE}
library(stringi)
